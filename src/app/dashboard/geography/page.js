                    ...
                    {topLocationsWidgetOpen && (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1">
                          Country:{" "}
                          <Tooltip title="Nigeria" placement="right">
                            <span className="drop-shadow text-[1.5em]">NG</span>
                          </Tooltip>
                        </div>
                        <div>City: {data.topCities[0].city}</div>{" "}
                        {/*TODO: Calculate top city from dateRange*/}
                      </Sheet>
                    )}
                  </div>
                </div>
...
const Map = ({ records, dateRange }) => {
  const [mapCenter, setMapCenter] = useState([3.3105, 6.5414]); // Default center
  const [mapZoom, setMapZoom] = useState(9); // Default zoom

  // Convert records to coordinates
